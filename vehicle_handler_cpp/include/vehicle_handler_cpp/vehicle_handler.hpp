...
public:
   ...
   * @param odom Odometry (COG)
   * @param steering_angle steering angle
   * @param wheelspeeds DatePerWheel wheel speeds
   * @return tam::types::common::DataPerWheel<double>
   */
  ...
  /**
   * @brief Estimates vertical forces on each tire.
   * Includes static load, aero forces, longitudinal and lateral load transfers.
   * @param velocity_mps Vehicle velocity vector at CoG [m/s].
   * @param ax_mps2 Longitudinal acceleration at CoG [m/s^2] (positive forward).
   * @param ay_mps2 Lateral acceleration at CoG [m/s^2] (positive results in load transfer L->R).
   * @return tam::types::common::DataPerWheel<double> Estimated vertical tire forces [N] (positive
   * downwards).
   *
   */
  tam::types::common::DataPerWheel<double> estimate_vertical_forces(
    const tam::types::common::Vector3D<double> & velocity_mps, double ax_mps2,
    double ay_mps2) const;
};
}  // namespace tam::common
