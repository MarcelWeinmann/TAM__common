...
{
  return tam::helpers::aerodynamics::eval_model(velocity, vehicle_.aero);
}
...
VehicleHandler::calc_long_slip_vectorized(
  const std::vector<double> & vx, const std::vector<double> & vy,
  const std::vector<double> & yawrate, const std::vector<double> & steering_angle,
  const std::vector<double> & wheelspeeds_fl, const std::vector<double> & wheelspeeds_fr,
  const std::vector<double> & wheelspeeds_rl, const std::vector<double> & wheelspeeds_rr) const
{
  // Step 1: Check if all vectors are of the same length
  size_t size = vx.size();
  if (
    wheelspeeds_fl.size() != size || wheelspeeds_fr.size() != size ||
    wheelspeeds_rl.size() != size || wheelspeeds_rr.size() != size) {
    throw std::invalid_argument(
      "All input vectors (vx, wheelspeeds_fl, wheelspeeds_fr, wheelspeeds_rl, wheelspeeds_rr) must "
      "be of the same length.");
  }

  // Step 2: Initialize result vectors
  std::vector<double> fl_slip;
  std::vector<double> fr_slip;
  std::vector<double> rl_slip;
  std::vector<double> rr_slip;

  fl_slip.reserve(size);
  fr_slip.reserve(size);
  rl_slip.reserve(size);
  rr_slip.reserve(size);
  ...
  for (size_t i = 0; i < size; ++i) {
    ...
    // Create and populate the wheelspeeds DataPerWheel
    tam::types::common::DataPerWheel<double> wheelspeeds;
    wheelspeeds.front_left = wheelspeeds_fl[i];
    wheelspeeds.front_right = wheelspeeds_fr[i];
    wheelspeeds.rear_left = wheelspeeds_rl[i];
    wheelspeeds.rear_right = wheelspeeds_rr[i];

    // Call the existing calc_long_slip function
    tam::types::common::DataPerWheel<double> slip = calc_long_slip(odom, steer, wheelspeeds);

    // Unpack the results into respective vectors
    fl_slip.push_back(slip.front_left);
    fr_slip.push_back(slip.front_right);
    rl_slip.push_back(slip.rear_left);
    rr_slip.push_back(slip.rear_right);
  }

  return std::make_tuple(fl_slip, fr_slip, rl_slip, rr_slip);
}
...
{
  return tam::helpers::vehicle_dynamics::slip_angle(
    odom, steering_angle, vehicle_.dimension, tires_);
}
std::tuple<std::vector<double>, std::vector<double>, std::vector<double>, std::vector<double>>
...
tam::types::common::DataPerWheel<double> VehicleHandler::estimate_vertical_forces(
  const tam::types::common::Vector3D<double> & velocity_mps, double ax_mps2, double ay_mps2) const
{
  tam::types::common::DataPerWheel<double> fz_tire_N =
    tam::helpers::vehicle_dynamics::tire_vertical_load(velocity_mps, ax_mps2, ay_mps2, vehicle_);
