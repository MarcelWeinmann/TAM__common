...
int main()
{
  ...
  tam::helpers::euler_rotations::Eigen3D a_2d =
    tam::helpers::euler_rotations::vector_to_2d(a_3d, pitch, roll);

  Eigen::VectorXd x = Eigen::VectorXd::LinSpaced(roll.size(), 0, roll.size());

  Eigen::VectorXd a_2d_ref =
    (ay_3d.array() * roll.array().cos()).matrix() - (az_3d.array() * roll.array().sin()).matrix();
