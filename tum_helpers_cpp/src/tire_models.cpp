         ...
         (params.lon.D *
          std::sin(
            ...
            std::atan(
              ...
              params.lon.E * (params.lon.B * slip_ratio - std::atan(params.lon.B * slip_ratio)))));
}
  ...
  const tam::types::tire_models::MF_52 & params) const
{
  ...
  const double S_V_x = F_z * (params.PVX1 + params.PVX2 * dfz) * params.LVX * params.LMUX;
  ...
  const double E_x = (params.PEX1 + params.PEX2 * dfz + params.PEX3 * std::pow(dfz, 2)) *
                     (1.0 - params.PEX4 * std::copysign(1.0, kappa_x)) * params.LEX;
  const double K_x =
    F_z * (params.PKX1 + params.PKX2 * dfz) * std::exp(params.PKX3 * dfz) * params.LKX;
  ...
  const tam::types::tire_models::MF_52 & params) const
{
  ...
  const double S_V_y_g =
    F_z * (params.PVY3 + params.PVY4 * dfz) * gamma_s * params.LKYG * params.LMUY;
  const double S_V_y = F_z * (params.PVY1 + params.PVY2 * dfz) * params.LVY * params.LMUY + S_V_y_g;
  const double K_y =
    params.PKY1 * F_z_o_a *
    std::sin(
      params.PKY4 *
  ...
  const double S_H_y =
    (params.PHY1 + params.PHY2 * dfz) * params.LHY + (K_y_g_o * gamma_s - S_V_y_g) / K_y;
  const double C_y = params.PCY1 * params.LCY;
  const double mu_y =
    ((params.PDY1 + params.PDY2 * dfz) / (1.0 + params.PDY3 * std::pow(gamma_s, 2))) * params.LMUY;
  ...
  const double F_y_o =
    ...
    S_V_y;
  // Combined slip
  const double D_V_y_k = mu_y * F_z * (params.RVY1 + params.RVY2 * dfz + params.RVY3 * gamma_s) *
                         std::cos(std::atan(params.RVY4 * slip_angle));
  const double S_V_y_k =
    D_V_y_k * std::sin(params.RVY5 * std::atan(params.RVY6 * slip_ratio)) * params.LVYKA;
  ...
  const double B_y_k = (params.RBY1 + params.RBY4 * std::pow(gamma_s, 2)) *
