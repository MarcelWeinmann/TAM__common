...
public:
  Receiver(const std::string & node_name, const rclcpp::NodeOptions & options)
  : Node(node_name, options)
  {
  }
  explicit Receiver() : Receiver("test_receiver", rclcpp::NodeOptions()) {}
  ~Receiver()
  {
  ...
  void add_processer(const std::string & topic, std::unique_ptr<BaseReceiver> && protocol_strategy)
  {
    ...
    receiver->recvMsgPubs = this->create_publisher<T>(topic, tam::ros::get_qos());
...
private:
  ...
  void receive_msgs(std::shared_ptr<RosMsgReceiver> receiver)
  {
    ...
    typed_ros2_pub =
      std::dynamic_pointer_cast<typename rclcpp::Publisher<T>>(receiver->recvMsgPubs);
